package turtleCode
{
	...
	public class Turtle extends Sprite
	{
		...
		public var pShape:Shape;// display object for graphics
		...
		private var pAlpha:Number;// alpha value of pen stroke
		...
		public function Turtle(aX:Number, aY:Number, aDelay:Number=500, aColor:Number= 0x0000FF, aWidth:Number=1)
		{
			...
			pTimer.addEventListener(TimerEvent.TIMER, doCommand);
			pTimer.start( );
			pList = new Array( );
			pTurtle.x = aX;
			pTurtle.y = aY;
		...
		public function beginFill(col:Number, alph:Number):void
		{
			...
			pShape.graphics.beginFill(col, alph);
		}
		...
		public function setStroke(aColor:Number, aWidth:Number, alph:Number):void
		{
			pColor = aColor;
			pWidth = aWidth;
			pAlpha = alph;
			pShape.graphics.lineStyle(aWidth, aColor, alph);
		}
		...
		private function doCommand(e:TimerEvent ):void
		{
			var theList = pList;
			if (pList.length > 0)
			{
		...
		public function moveAbs(aX:Number, aY:Number):void
		{
			...
			if (pPen)
			{
				...
				pShape.graphics.lineTo(aX, aY);
			}
			else
			{
				//pList.push({op:"moveTo", x:aX, y:aY});
				pShape.graphics.moveTo(aX, aY);
			}
		}
